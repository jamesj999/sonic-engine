...
- Gap: Only subset implemented; fades/comms stubs. See `SmpsSequencer.handleFlag`, `handleFadeIn` (stop). Reference flags: `docs/SMPS-rips/Sonic The Hedgehog 2/DefCFlag.txt`.
...
- Gap: Voices reordered + op2/op3 swapped in `src/main/java/uk/co/jamesj999/sonic/audio/smps/Sonic2SmpsData.java#getVoice`. SMPSPlay uses `InsMode=Hardware` (`DefDrv.txt`).
- Fix: Support hardware-order load for Sonic 2; gate swaps/remaps by driver metadata.

## DAC Rates & Playlist
- Gap: Always interpolate; no playlist aliases. Timing constants in `src/main/java/uk/co/jamesj999/sonic/audio/synth/Ym2612Chip.java` (DAC_*). Loader maps only 0x81–0x91 (`Sonic2SmpsLoader.loadDacData`). Reference: `docs/SMPS-rips/Sonic The Hedgehog 2/DAC.ini`, `DefDrum.txt` aliases.
...
- Gap: Envelopes hardcoded (`Sonic2PsgEnvelopes.java`); noise/pan simplified. PSG handling in `SmpsSequencer` PSG functions and `src/main/java/uk/co/jamesj999/sonic/audio/synth/PsgChip.java`. Reference: `docs/SMPS-rips/Sonic The Hedgehog 2/PSG.lst`; SN76496 clock in SMPSPlay is 3.579545 MHz with tone `f = clock/(32*N)`, which corresponds to `CLOCK_DIV = 8` in our chip; comment currently mismatches.
...
- Gap: Simplified pitch wrap (`getPitchSlideFreq`, `applyModulation`). Driver: `DefDrv.txt` (DetuneOctWrap=False, ModAlgo=68k_a) and `DefCFlag.txt`.
...
- Gap: Global `dividingTiming`; per-track multipliers not fully matched. Code: `SmpsSequencer.setTrackDividingTiming/updateDividingTiming`. Reference: `DefDrv.txt` (DefTimingMode=00, LockTimingMode=True).
- Fix: Enforce per-track tick multipliers as driver; lock timing mode per driver settings.

## SFX Coverage & Routing
- Gap: Only RING mapped; single SMPS SFX stream; other SFX fall back to WAV. Code: `JOALAudioBackend.playSfxSmps/playSfx`, `AudioManager.playSfx`. References: `docs/SMPS-rips/Sonic The Hedgehog 2/SFX_Final/*.sfx`, `DefCFlag.txt`.
...
- Gap: Hardcoded music map; limited SFX. Code: `Sonic2SmpsLoader.musicMap`, `resolveMusicOffset`, `loadSfx`. Reference: `docs/SMPS-rips/Sonic The Hedgehog 2/Pointers.txt`, `Z80Drv_0925_Final.bin` offsets.
...
- Gap: Custom YM2612/PSG approximate (mono mix, simplified EG/LFO, soft clip, minimal pan); SMPSPlay uses mature cores. Files: `VirtualSynthesizer.java`, `Ym2612Chip.java`, `PsgChip.java`, `JOALAudioBackend.fillBuffer`.
...
- Gap: Pan/AMS/FMS applied on note-on; no FM6 DAC-off option. Code: `SmpsSequencer.applyFmPanAmsFms`, `JOALAudioBackend.updateSynthesizerConfig`. Reference: `DefDrv.txt` (FM6DACOff optional).
- Fix: Respect FM6 DAC-off; ensure pan bits map to stereo output.

## Fade Handling & Master Volume
- Gap: No real fades/master gain. Code: `SmpsSequencer.handleFadeIn/handleFlag`. Reference: fade params in `DefDrv.txt` (FadeOutSteps/Delay, FadeInSteps/Delay).
