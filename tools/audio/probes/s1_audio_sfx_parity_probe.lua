-- Read-only Sonic 1 REV01 sound-test SFX observer.
-- Captures the same per-UpdateMusic-invocation record shape as the GHZ music
-- parity probe (driver RAM music-slot state + ordered YM/PSG bus writes), over
-- the pinned s1-soundtest-sfx.bk2 movie: GHZ music starts exactly as in the
-- music capture (the movie shares its full 989-row prefix), then eight normal
-- SFX ($A0 $A4 $A6 $AA $B5 $C6 $CC $CF) are played from the sound test over
-- the running music. Each Sound_PlaySFX dispatch is recorded on the invocation
-- it happened in (the "dispatches" array), which is the request-sequence input
-- contract the engine-side capture replays. There is no recurrence proof:
-- the capture ends deterministically at movie end (cycle_start 0, period 0).
--
...
local EXPECTED_MOVIE_ROWS = 2791
...
local EXPECTED_MOVIE_SHA256 = "3da775e4fdd3770e9687e178b6a11922873d1021a2013565ebe842451a7a33a2"
...
local function verifyIdentity()
    ...
    assert(movie.length() == EXPECTED_MOVIE_ROWS,
        "pinned S1 sound-test SFX BK2 must have 2,791 input rows")
...
local function assertStableSoundTest()
    -- The SFX movie stays on the sound-test row but changes the selected
    -- number and queues SFX requests, so only mode and the commands the movie
    -- never sends are contamination.
...
local function emitCapture(context)
    ...
    context.log(AudioContract.canonicalJson({
        capture = "s1_soundtest_sfx_driver_reference",
...
ProbeRuntime.run({
    ...
    onFrame = function(context)
        ...
        if not VALIDATE_ONLY and not emitted and context.movieFinished()
                and not invocationLifecycle:isActive() then
            assert(invocationLifecycle:isArmed(), "movie finished before the GHZ capture epoch")
            assert(#records > 0, "movie finished with no captured invocations")
            emitted = true
            emitCapture(context)
        end
        assert(#records < 36000, "SFX capture exceeded the 36,000-invocation budget")
    end
})
