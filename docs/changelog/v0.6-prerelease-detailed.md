...
- **docs(audio): resolve the complete S3K SMPS meta-command finding with ROM reachability
  ...
  `Sonic3kCoordFlagHandler` cases consume operands only to preserve alignment
  for custom/imported streams; they do not claim native sound dispatch,
  all-track halt/resume, or shared-Z80-memory copying. The ROM-backed guard and
  native Z80 contracts are recorded in
...
- **S3K sidekick bootstrap parity (CNZ/MGZ trace replay).** Three S3K
  ...
  MGZ1 / HCZ1 / LRZ1 falling intros. SSZ has no corresponding
  `SpawnLevelMainSprites` branch. Fix preserves the zone-event
