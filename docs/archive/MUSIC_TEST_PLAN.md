...
Context: FM instruments remain flaky, DAC drums vary (kick okay, snare noisy), and several tracks misbehave (loops/tempo/section jumps). Recent change: F9 now returns from F8 calls (was ending tracks), so re-check intros/loops.

## Goals
- Use the sound-test to A/B songs and inspect channel data without gameplay.
...
- Looping/tempo/section errors (0x84 Mystic Cave, 0x8F Oil Ocean, 0x99 Invincibility):
  ...
  - Coordination flags now consume Sonic 2 param lengths and F9 is handled as return (not stop); per-track tick multiplier (E5) is wired but its effect still needs hardware validation.
...
- Implement/verify remaining flags: E5 behaviour, E1/E2 detune placeholders, and ensure F6/F8/F7 flows do not prematurely end tracks.
...
- Coordination flags: log `E0/E6/E7/E8/E9/EA/EB/F0/F2/F6/F7/F8/F9/F3/F5` when hit; verify tempo/dividing-timing updates mid-playback (call/return already fixed).
...
2) Validate E5 behaviour/tempo interactions in-engine; re-test 0x84/0x8F/0x99 intros/loops now that F9 is fixed.
