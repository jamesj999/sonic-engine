...
public final class AudioPresentationSourceFactory
        ...
        if (descriptor.route() == AudioSourceDescriptor.Route.BASE_MUSIC_ID
            ...
            return streamedStockOverride(voiceId, musicId, descriptor,
                    StreamedMusicVoice.RestorePolicy.from(
                            source.staticConfig(), !streamedRestorePolicy.releaseOnRestore()));
        }
        if (smpsSession == null) {
            throw new IllegalStateException(
    ...
    /**
     * Takes the launch-scoped streamed-music port that creator overrides play
     * through. The port is owned by the mod session, not by this factory, so it
    ...
    public void setStreamedRestoreConfig(
            SmpsSequencerConfig config, boolean blockSfxDuringFade) {
        streamedRestorePolicy = config == null
                ? StreamedMusicVoice.RestorePolicy.immediate()
                : StreamedMusicVoice.RestorePolicy.from(config, blockSfxDuringFade);
    }
    ...
    /**
     * Builds the presentation entry for a creator-supplied streamed override of
    ...
    /**
     * Builds the presentation entry for a namespaced creator track. Namespaced
     * tracks override no stock id, so the entry carries {@code -1}.
     */
    ...
    /**
     * Builds a creator one-shot as an ordinary sample voice. Registering the PCM
     * under its owner-scoped key means the voice snapshots and restores through
     * the same cache path as stock fallback SFX, so a rewind over a mod SFX
     * behaves like a rewind over any other sample voice.
     */
    public SampleBackedVoice streamedSfx(
        ...
        DecodedPcm pcm = settings.pcmCache().register(new DecodedPcm(
                assetId, source.channels(), source.sampleRate(), source.samples()));
        return SampleBackedVoice.oneShot(voiceId, priority, pcm,
                roundedOutputSampleRate(), 1.0f, source.gain());
    }
