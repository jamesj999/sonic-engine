...
public class Sonic2TrackAnimator {
    ...
    public void initialize(int stageIndex) throws IOException {
        ...
        if (layoutLength > 0) {
            ...
            currentSegmentType = Sonic2SpecialStageConstants.SEGMENT_STRAIGHT;
    ...
    public void initializeWithMockLayout() {
        stageLayout = new byte[] {
            Sonic2SpecialStageConstants.SEGMENT_STRAIGHT_THEN_TURN,
            Sonic2SpecialStageConstants.SEGMENT_TURN_THEN_RISE,
            Sonic2SpecialStageConstants.SEGMENT_TURN_THEN_STRAIGHT,
            Sonic2SpecialStageConstants.SEGMENT_STRAIGHT,
            Sonic2SpecialStageConstants.SEGMENT_STRAIGHT_THEN_TURN,
            Sonic2SpecialStageConstants.SEGMENT_TURN_THEN_DROP,
            (byte)(Sonic2SpecialStageConstants.SEGMENT_TURN_THEN_STRAIGHT | 0x80),
            Sonic2SpecialStageConstants.SEGMENT_STRAIGHT,
        };
    ...
    private boolean advanceFrame() {
        int[] animation = Sonic2SpecialStageConstants.SEGMENT_ANIMATIONS[currentSegmentType];
    ...
    private void parseCurrentSegment() {
        ...
        if (currentSegmentType >= Sonic2SpecialStageConstants.SEGMENT_ANIMATIONS.length) {
    ...
    private void loadStageLayout(byte[] allLayouts, int stageIndex) {
        ...
        if (stageIndex < 0 || stageIndex >= Sonic2SpecialStageConstants.SPECIAL_STAGE_COUNT) {
        ...
        int nextOffset;
        if (stageIndex < Sonic2SpecialStageConstants.SPECIAL_STAGE_COUNT - 1) {
    ...
    public int countInitialStraightSteps() {
        ...
        for (int i = 0; i < layoutLength; i++) {
            int segByte = stageLayout[i] & 0xFF;
            int type = segByte & 0x7F;
            if (type == Sonic2SpecialStageConstants.SEGMENT_STRAIGHT) {
                steps += 16; // STRAIGHT = 16 animation steps
            } else if (type == Sonic2SpecialStageConstants.SEGMENT_STRAIGHT_THEN_TURN) {
