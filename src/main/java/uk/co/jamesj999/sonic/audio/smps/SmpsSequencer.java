...
public class SmpsSequencer implements AudioStream {
    ...
    private class Track {
        ...
        int baseFnum;
        int baseBlock;
        final int[] loopCounters = new int[16];
        int loopTarget = -1;
        ...
        return switch (cmd) {
            ...
            case 0xF6, // Jump
                 0xF8  // Call
                    -> 2;
            // 3-byte params
            // case 0xF7 -> 3; // Moved to 4-byte params

            // 4-byte params
            case 0xF0, // Modulation setup
                 0xF7  // Loop (idx + count + ptr)
                    -> 4;
            // 0-byte params
            case 0xE3, // Return
                 0xE4, // Fade in
    ...
    private void handleLoop(Track t) {
        if (t.pos + 4 <= data.length) {
            ...
            // Index 0-15 supported. Wrap if needed or clamp.
            index &= 0xF;
    ...
    public synchronized DebugState debugState() {
        ...
        for (Track t : tracks) {
            ...
            dt.loopCounters = t.loopCounters.clone();
            dt.position = t.pos;
            state.tracks.add(dt);
        }
        return state;
