...
class TestRomByteReaderCache {
    ...
    @Test
    void logicalRomWindowCopiesOnlyItsBoundedBytesAndRetainsSnapshotIsolation() {
        byte[] source = {9, 1, 2, 8};
        RomByteReader window = RomByteReader.fromBytes(source, 1, 3);
        source[1] = 99;
        assertEquals(2, window.size());
        assertEquals(0x0102, window.readU16BE(0));
        assertThrows(IndexOutOfBoundsException.class,
                () -> RomByteReader.fromBytes(source, 1, 5));
    }
